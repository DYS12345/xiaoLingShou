...
- (void)startRequestSuccess:(void (^)(FBRequest *, id))success failure:(void (^)(FBRequest *, NSError *))failure {
    ...
    if (self.requestType) {
        ...
        [self.manager.requestSerializer setValue:@"application/json; charset=gb2312" forHTTPHeaderField:@"Content-Type" ];
        self.manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json",@"text/html", @"text/plain",nil];
    } else {
        self.manager.requestSerializer = [FBRequest requestSerializerWith:HTTPRequestType];
    }
    ...
    if (self.RequestMethod == GET_METHOD) {
        ...
        __weak FBRequest *weakSelf = self;
        NSLog(@"--Parameters: %@", [weakSelf transformRequestDictionary]);
        NSString *str = [weakSelf transformRequestDictionaryStr];
        NSDictionary *d = [NSString dictionaryWithJsonString:str];
        self.httpOperation = [self.manager GET:self.urlString
                                    parameters:d
//                                      progress:^(NSProgress * _Nonnull downloadProgress) {
//            
...
- (NSString *)transformRequestDictionaryStr {
    return self.requestDictionaryStr;
}
                        ...
                        responseType:(AFNetworkingResponseType)responseType 
{
    ...
    if (model != nil) {
        ...
        appSecret = model.access_token;
    }
    NSString *timestamp = [FBRequest getNowTimeTimestamp];
    NSString *nonce_str = [[NSUUID UUID] UUIDString];//随机字符串;
    NSString *sign1 = [NSString stringWithFormat:@"app_key=%@&nonce_str=%@&timestamp=%@%@", app_key, nonce_str, timestamp, appSecret];
    sign1 = [sign1 stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSString *sign = [NSString sha1:sign1];
    sign = [sign stringByReplacingOccurrencesOfString:@" " withString:@""];
    NSDictionary *dict = @{@"app_key" : app_key,
                           @"timestamp" : timestamp,
                           @"nonce_str" : nonce_str,
                           @"sign" : sign
                           };
    NSMutableDictionary *dict1 = [NSMutableDictionary dictionaryWithDictionary:dict];
    NSArray *keys = requestDictionary.allKeys;
    for (NSString *key in keys) {
        dict1[key] = requestDictionary[key];
    }
    ...
    request.requestDictionary = dict1;
    ...
    request.requestDictionaryStr = [NSString convertToJsonData:dict1];
    
    NSLog(@"token %@", model.token);
    if (model.token.length == 0) {
        NSString *str = [NSString stringWithFormat:@"%@:%@", dict1[@"email"], dict1[@"password"]];
        NSString *str1 = [NSString stringWithFormat:@"Basic  %@", [NSString base64StringFromText:str]];
        NSString *str2 = [str1 stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        request.HTTPHeaderFieldsWithValues = @{@"Authorization" : str2};
    } else {
        NSString *str = [NSString stringWithFormat:@"%@:%@", model.token, model.token];
        NSString *str1 = [NSString stringWithFormat:@"Basic  %@", [NSString base64StringFromText:str]];
        NSString *str2 = [str1 stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
        request.HTTPHeaderFieldsWithValues = @{@"Authorization" : str2};
    }
